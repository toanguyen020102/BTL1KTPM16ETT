...
public class GUI_TimSanPham_2 extends JPanel implements ActionListener{
	...
	public GUI_TimSanPham_2(JFrame frame) {
	...
	private void formDisplayProduct(String tenSanPham) {
		// TODO Auto-generated method stub
		frameForm = new JFrame();
		frameForm.setLayout(new BorderLayout());
		frameForm.setTitle("" + tenSanPham);
		frameForm.setSize(600, 800);
		...
		bChiTietSanPham = Box.createVerticalBox();
		Box bChiTietSanPhamHeader = Box.createHorizontalBox();
		String s = "";
		s += s.format("%20s%20s%20s%20s", "Kích Cỡ", "Màu Sắc", "Số Lượng", "Giá Bán");
		bChiTietSanPhamHeader.add(new JLabel("" + s));
		bChiTietSanPham.add(bChiTietSanPhamHeader);
		pnlForm.add(bChiTietSanPham);
	...
	private void activeFormDisplayProduct(String maSanPham) {
		...
		for(SanPham sanPham : sanPhamArray) {
			if(sanPham.getMaSP().equalsIgnoreCase(maSanPham)) {
				lblAnh = new JLabel(new ImageIcon(scaleImage(sanPham.getPathImage() + sanPham.getMaSP() + ".jpg", 400, 400)));
				formDisplayProduct(sanPham.getTenSP());
				txtTenSanPhamDisplay.setText("" + sanPham.getTenSP());
				txtTenThuongHieuDisplay.setText("" + sanPham.getTenTH());
				txtNuocThuongHieuDisplay.setText("" + sanPham.getNuocTH());
				txtChatLieuDisplay.setText("" + sanPham.getChatLuong());
				txtKieuDangDisplay.setText("" + sanPham.getKieuDang());
				txaMoTa.setText("" + sanPham.getMoTa());

				chiTietSanPhamDAO = new ChiTietSanPham_DAO();
				chiTietSanPhamArray = chiTietSanPhamDAO.getAllChiTietSanPhamTheoMaSanPham(maSanPham);
				for(ChiTietSanPham chiTietSanPham : chiTietSanPhamArray) {
					String s = "";
					s += s.format("%20s%20s%20s%20s", chiTietSanPham.getKichCo(), chiTietSanPham.getMauSac(), chiTietSanPham.getSoLuong(), chiTietSanPham.getGiaBan());
